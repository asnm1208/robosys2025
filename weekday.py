...
def is_valid_date(y: int, m: int, d: int) -> bool:
    if y <= 0 or not (1 <= m <= 12) or d <= 0:
        return False
    mdays = [31, 29 if is_leap_year(y) else 28, 31, 30,
...
def process_line(line: str):
    ...
    try:
        ...
        print(f"ERROR: 年/月/日は整数で指定してください: '{line}'", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    if not data:
        print("Usage: 標準入力から 'YYYY M D' を1行に1件で与えてください。複数行でも可。")
        print("例:\n2025 1 1\n2100 2 29\n(EOFで実行)")
        return 0
